...
const XUNIT = new ARR([1, 0, 0]);

export class Vec3 {

  static create(x = 0, y = 0, z = 0) {
    return new ARR([x, y, z])
  }

  static normalize(out, a) {
    let x = a[0], y = a[1], z = a[2];
    let placeholder = (x * x) + (y * y) + (z * z);
    if (placeholder > 0) {
      placeholder = 1 / Math.sqrt(placeholder);
    }
    out[0] = a[0] * placeholder;
    out[1] = a[1] * placeholder;
    out[2] = a[2] * placeholder;
    return out;
  }
...
*/
  ...
  static rotationTo(out, a, b) {
    let _dot = Vec3.dot(a, b);
    let tmpvec3 = Vec3.create();
    if (_dot < -0.999999) {
      Vec3.cross(tmpvec3, XUNIT, a);
      if (Vec3.len(tmpvec3) < 0.000001) Vec3.cross(tmpvec3, YUNIT, a);
      Vec3.normalize(tmpvec3, tmpvec3); 
      Quat.setAxisAngle(out, tmpvec3, Math.PI);
      return out;
    } else if (_dot > 0.999999) {
      out[0] = 0;
      out[1] = 0;
      out[2] = 0;
      out[3] = 1;
      ...
      out[2] = tmpvec3[2];
      out[3] = 1 + _dot;
      //Quat.setAxisAngle(out, tmpvec3, Math.cos(_dot));
      //return out;
